...
def PlotData(df, p = None):
    ...
    mpl.plot(ts, df.Close.values, color = 'r', linewidth = 1, label = 'Actual')
    #Predicted data was also provided
    if(len(p) > 0):
        mpl.plot(ts[p], df.Close.values[p], color = 'g', linewidth = 1, label = 'Predicted')
    #Set the tick marks
...
def PlotTestData(df, actual):
    ...
    print(len(actual.Close.values))
    print(len(ts))

    mpl.plot(ts, actual.Close.values, color = 'r', linewidth = 1, label = 'Actual')
    #Predicted data was also provided
    mpl.plot(ts, df.High.values, color = 'g', linewidth = 1, label = 'Predicted')
    #Set the tick marks
    mpl.xticks(tickMarks, strTs, rotation='vertical')
    #Set y-axis label
    mpl.ylabel('Stock Close Value (USD)')
    #Add the label in the upper left
